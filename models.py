...
class AirQualityResult(BaseModel):
    ...
    def observation_date_time(self):
        dt = datetime.combine(self.date_observed, datetime.min.time()).replace(hour=self.hour_observed)
...
class AirQualityResultsList(BaseModel):
    ...
    @property
    def report(self):
        analysis = "Everything looks good!" if self.is_healthy else "Air quality doesn't look great!"
